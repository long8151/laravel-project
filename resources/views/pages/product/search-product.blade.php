      ...
      <div class="row">
        <div class="col-2">
          <div class="left-item category-item">
            ...
            <a href="{{URL::to('show-product-by-category'.'/'.$cat_item->category_id)}}" class="">{{$cat_item->category_name}}</a>
            @endforeach
          </div>
          <div class="left-item brand-item">
            <h5>Brand</h5>
            @foreach($brand_item as $key => $bra_item)
            <a href="{{URL::to('show-product-by-brand'.'/'.$bra_item->brand_id)}}" class="">{{$bra_item->brand_name}}</a>
            @endforeach
          </div>
        </div>
        <div class="col-10">
          ...
          <div class="product-section">
            @foreach($search_item as $key => $search)
            <div class="product">
              <a href="{{URL::to('product-detail'.'/'.$search->product_id)}}"><img src="{{{'public/uploads/product/'.$search->image}}}" alt=""></a>
              <div class="d-flex">
              ...
              <div class="product-info">
                <h6><b>{{$search->product_name}}</b></h6>
                <p>{{$search->category_name}}</p>
                <h5><b>{{$search->price}}$</b></h5>
              </div>
            </div>
            @endforeach
          </div>
